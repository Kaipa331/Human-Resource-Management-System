-- Fixed Payroll System Database Schema
-- This script ensures all tables and columns exist properly
...
CREATE INDEX idx_payroll_cycle_id ON payroll(cycle_id);
...
CREATE INDEX idx_training_employee_id ON training_enrollments(employee_id);
CREATE INDEX idx_training_course_id ON training_enrollments(course_id);
CREATE INDEX idx_training_status ON training_enrollments(status);
...
-- Initialize with sample training courses
...
BEGIN
    ...
    RAISE NOTICE 'All columns properly created including pay_date';
END $$;
